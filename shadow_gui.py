...
from shadow_generator import (
    _load_image, _ensure_bgr, _load_or_create_mask,
    _place_on_canvas, _compute_contact_line, _build_shadow,
    _apply_depth_warp, _composite
)
...
class ShadowGeneratorGUI:
    ...
    def _save_outputs(self):
        ...
        if self.mask is not None:
            mask_canvas = _place_on_canvas(self.mask, self.bg.shape, 
                                          (self.bg.shape[1] - self.mask.shape[1]) // 2,
                                          self.bg.shape[0] - self.mask.shape[0])
